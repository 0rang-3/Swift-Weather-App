...
def update_string(): #HTML Form to Choose City
    ...
    aqi = weatherList[7]
    aqi_converted = int(aqi)*12
    aqi_recommendation = weatherList[8]

    #Render weather.html
    return render_template("weather.html", temp = temp_fahrenheit_rounded, feelslike = feels_like_fahrenheit_rounded, speed = speed_mph, dir = direction, hum = humidity,  desc = description, rec = recommendation, aqi = aqi_converted, aqi_rec = aqi_recommendation, city = CITY)
